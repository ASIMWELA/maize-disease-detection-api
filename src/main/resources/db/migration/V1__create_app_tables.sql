...
create table file_system_image
(
    id         int8 generated by default as identity,
    uuid       varchar(12)  not null,
...
alter table if exists file_system_image
    add constraint UK_j0fb8oqw41nh2gc5m7sq740j1 unique (answer_id);

alter table if exists file_system_image
    add constraint UK_j16mi4towp3k60dx6aq92o1kg unique (issue_id);
...
create index user_uuid_index on users_table (uuid);
...
alter table if exists file_system_image
    add constraint FKuj67ipflv9jj65f7jm90mv7f
    foreign key (answer_id)
    references answers_table;

alter table if exists file_system_image
    add constraint FK1h5rk6w4yt7k5pboab8sx9fi
    foreign key (issue_id)
    references issues_table;
